...
ReadRCExtraceCmd::ReadRCExtraceCmd(const std::string &name, FanMgr *fanMgr) : Cmd(name) {
    fanMgr_ = fanMgr;
    optMgr_.setName(name);
    optMgr_.setShortDes("read rc extrace");
    optMgr_.setDes("read rc extrace form FILE");
    optMgr_.regArg(new Arg(Arg::REQ, "rc extrace file", "FILE"));
    Opt *opt = new Opt(Opt::BOOL, "print usage", "");
    opt->addFlag("h");
    opt->addFlag("help");
    optMgr_.regOpt(opt);
...
ReadRCExtraceCmd::~ReadRCExtraceCmd() {}
//}}}
//{{{ bool ReadRCExtraceCmd::exec()
bool ReadRCExtraceCmd::exec(const vector<string> &argv) {
    optMgr_.parse(argv);
    ...
    if (!fanMgr_->atpg_mgr->cir_) {
        cerr << "**ERROR ReadRCExtraceCmd::exec(): circuit needed";
        cerr << endl;
        return false;
    }
	
    if (optMgr_.getNParsedArg() < 1) {
        cerr << "**ERROR ReadRCExtraceCmd::exec(): rc extrace file needed";
        cerr << endl;
        return false;
    }
	
	cout << "optMgr_.getParsedArg(0).c_str() " << optMgr_.getParsedArg(0).c_str() << endl;
	...
	char line[SIZE];
	string temp_string;
	ifstream frcextracefile(optMgr_.getParsedArg(0).c_str());
	...
	while(frcextracefile.getline(line,sizeof(line),' ')){
		if(line[0] == '*'){
			...
			int aggrgate=0;
			char* line_ = &line[1];
			const char* cline_ = line_;
			aggrgate = atoi(cline_);
...
ReportRCExtraceCmd::ReportRCExtraceCmd(const std::string &name, FanMgr *fanMgr) :
  Cmd(name) {
    fanMgr_ = fanMgr;
    optMgr_.setName(name);
    optMgr_.setShortDes("report rc extrace");
    optMgr_.setDes("report rc extrace information");
    Arg *arg = new Arg(Arg::REQ_INF, "rc extrace number to be reported", "NUM");
    optMgr_.regArg(arg);
    Opt *opt = new Opt(Opt::BOOL, "print usage", "");
    opt->addFlag("h");
    opt->addFlag("help");
...
ReportRCExtraceCmd::~ReportRCExtraceCmd() {}
//}}}
//{{{ bool ReportRCExtraceCmd::exec()
bool ReportRCExtraceCmd::exec(const vector<string> &argv) {
    optMgr_.parse(argv);
...
bool AddFaultCmd::exec(const vector<string> &argv) {
    ...
    if (optMgr_.isFlagSet("a"))
        ...
        if (fanMgr_->atpg_mgr->fListExtract_->type_ == FaultListExtract::SAF
			...
			if (fanMgr_->atpg_mgr->fListExtract_->type_ == FaultListExtract::FT &&
                type != "FT") {
                cerr << "**ERROR AddFaultCmd::exec(): fault tuple only ";
                cerr << "supports fault tuple" << endl;
                return false;
            }
...
bool ReportFaultCmd::exec(const vector<string> &argv) {
    ...
    switch (fanMgr_->atpg_mgr->fListExtract_->type_) {
        ...
        case FaultListExtract::BRF:
            ...
            break;
        case FaultListExtract::FT:
            cout << "fault tuple" << endl;
            break;
        default:
            cout << endl;
            break;
